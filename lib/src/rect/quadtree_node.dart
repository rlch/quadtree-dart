...
class RectQuadtreeNode<O extends Rect> implements QuadtreeNode<O> {
  RectQuadtreeNode({
    Rect? extent,
    this.maxObjects = 10,
    this.maxDepth = 4,
  ...
  List<O> retrieveAllObjects() {
    final List<RectQuadtreeNode<O>> nodes = [this];
    final List<O> objects = [];
  ...
  List<Rect> retrieveAllNodes() {
    final List<RectQuadtreeNode<O>> next = [this];
  ...
  void clear() {
    ...
    for (final node in nodes ?? <RectQuadtreeNode<O>>[]) {
